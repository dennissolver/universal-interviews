  ...
  async function handleSingleInvite(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/invites/send', {
        ...
        body: JSON.stringify({ panelId, interviewees: [{ name: singleName, email: singleEmail }] })
      });
  ...
  async function handleBulkInvite() {
    ...
    try {
      const res = await fetch('/api/invites/send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ panelId, interviewees: bulkInterviewees })
      });
      const data = await res.json();
      if (res.ok && data.results) {
        setBulkResults(data.results);
        const failedEmails = data.results.filter((r: InviteResult) => !r.success).map((r: InviteResult) => r.email);
        ...
        <div className="max-w-2xl mx-auto px-4 py-8">
          ...
          <div className="flex gap-2 mb-6">
            <button
              ...
              className={`px-4 py-2 rounded-lg flex items-center gap-2 transition ${activeTab === 'single' ? 'bg-purple-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
            >
              <User className="w-4 h-4" />Single Invite
            </button>
            <button
              onClick={() => setActiveTab('bulk')}
              className={`px-4 py-2 rounded-lg flex items-center gap-2 transition ${activeTab === 'bulk' ? 'bg-purple-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
            >
          ...
          {activeTab === 'single' && (
            <div className="bg-slate-900 rounded-2xl p-6">
              <form onSubmit={handleSingleInvite} className="space-y-4">
                ...
                {singleResult && (
                  <div className={`p-3 rounded-lg flex items-center gap-2 ${singleResult.success ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
            ...
            <div className="space-y-6">
              <div className="bg-slate-900 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={downloadTemplate} className="text-sm text-purple-400 hover:text-purple-300 flex items-center gap-1">
              ...
              {bulkInterviewees.length > 0 && (
                <div className="bg-slate-900 rounded-2xl p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => setBulkInterviewees([])} className="text-sm text-slate-400 hover:text-red-400">
                      Clear All
                    </button>
                  </div>
                  <div className="max-h-64 overflow-y-auto space-y-2 mb-4">
                    {bulkInterviewees.map((person, i) => (
                      <div key={i} className="flex items-center justify-between bg-slate-800 rounded-lg px-3 py-2">
                        ...
                        <button
                          onClick={() => setBulkInterviewees(bulkInterviewees.filter((_, idx) => idx !== i))}
                ...
                <div className="bg-slate-900 rounded-2xl p-6">
                  ...
                  <div className="space-y-2">
                    {bulkResults.map((result, i) => (
                      <div
                        key={i}
                        className={`flex items-center gap-2 p-2 rounded-lg ${result.success ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}
                      >
